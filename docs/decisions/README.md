...
| D-015 | Use Tier 0, Tier 1, and Tier 2 as the primary dependency and recovery language | Accepted | Makes ownership and recovery boundaries explicit alongside functional layers | Capabilities flow upward from Tier 0 to Tier 2; Tier 0 remains in air-gapped custody with artifact handoffs rather than routed connections | If the tier model is replaced or a service must intentionally cross a tier boundary |
...
| D-018 | Keep one Terraform/Ansible inventory contract per tier and reusable playbooks in shared code | Accepted | Separates operational ownership while retaining one mechanism for names, IPs, and environment overlays | Three tier-owned inventories and state roots; no live inventory in shared; inventory applications remain Day 2 consumers | If inventory ownership, recovery inputs, or cross-tier orchestration changes |
